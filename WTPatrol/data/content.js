...
// var msg = "msg from content.js";


//  self.port.emit("logValue", msg);
...
function getPageScript() {
  ...
  return "(" + function () {
    ...
    var screenProperties =  [ "pixelDepth", "colorDepth", "orientation" ];
    ...
    var windowProperties = [ "name", "localStorage", "sessionStorage", "performance" ];
    ...
    var excludedProperties = [ "quadraticCurveTo", "lineTo", "transform",
                               ...
                               "beginPath", "canvas", "translate", "getContext" ];
    ...
    // Access to Battery API -- not available on mobile
    // instrumentObject(window.BatteryManager.prototype, "BatteryManager");

    //Instrument window.addEventListener
    var monitoredEventArr = ["devicelight", "devicemotion", "deviceorientation", "click"];
    ...
    Window.prototype.addEventListener = function () {
        ...
        arguments[1] = (function (userFunc) {
            return function instrumentAddEventListener() {
                var callContext = getOriginatingScriptContext(false);
                logCall("window.addEventListener()", [arguments[0].type], callContext, {});

                userFunc.apply(eventThis, arguments);
            }
        })(arguments[1]);
        return Window.prototype._addEventListener.apply(this, arguments);
    }
...
window.addEventListener('load', (event) => {
  setTimeout(start_scroll, 5  * 1000);
  var msg = {
        uri: window.location.href,
        page_src: document.documentElement.outerHTML
      };
  self.port.emit("logPageSrc", msg)
});

setTimeout(start_scroll, 10 * 1000);

var num_scroll = 0;
var body_height, scroll_flag = false;
function start_scroll(){
    if (!scroll_flag){
      ...
      setTimeout(continue_scroll, 1000)
    }
...
function continue_scroll(){
  ...
  // body_height = document.body.scrollHeight;
  body_height = getDocHeight();
  if (body_height == old_height || num_scroll > 5){ // end of scrolling
    var msg = {
        uri: window.location.href,
        page_src: document.documentElement.outerHTML
      };
