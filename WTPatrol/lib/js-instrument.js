...
var pageSrcArr = [], myworker, pageSrcPath;
...
exports.run = function(crawlID) {
    ...
    onAttach: function onAttach(worker) {
	    ...
	    function processCallsAndValues(data) {
      	...
      	worker.port.on("logPageSrc", function(data){pageSrcArr.push(data);});
    }
  });
};
...
exports.savePageSrcToFile = function(finalUrl, pageSrcPath){
  if(pageSrcArr.length == 0) return;
  var page_src="";
  for(var i = 0; i < pageSrcArr.length; i++){
    if(pageSrcArr[i].uri == finalUrl && pageSrcArr[i].page_src.length > page_src.length){
    	page_src = pageSrcArr[i].page_src;
    }
  }

  var fileName = crawlID + "_" + visitID + "_" + finalUrl.replace(/\//g, '*');
  respBodySaver.saveToLocalFile(fileName, page_src, pageSrcPath);
