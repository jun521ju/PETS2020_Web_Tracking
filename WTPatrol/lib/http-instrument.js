...
var httpPostParser    = require("./http-post-parser");
var respBodySaver    = require("./resp-body-saver");
...
var visitID = 0, crawlID = 0; // indicates currently visited website
...
function removeQuoteMarkinStr(str){
  var charArr = [];
...
var httpRequestHandler = function(reqEvent, crawlID) {
      ...
      if ("post_headers" in postObj) {
        ...
        for (var name in postObj["post_headers"]) {
          if (contentHeaders.includes(name)){
            ...
            header_pair.push(name);
            header_pair.push(postObj["post_headers"][name]);
            headers.push(header_pair);
          }
        }
      }
  ...
  update["headers"] = "'" + removeQuoteMarkinStr(JSON.stringify(headers)) + "'";
...
var httpResponseHandler = function(respEvent, isCached, crawlID, saveJavascript) {
  ...
  httpChannel.visitResponseHeaders({visitHeader: function(name, value) {
    ...
    // header_pair.push(loggingDB.escapeString(name));
    // header_pair.push(loggingDB.escapeString(value));
    // loggingDB.escapeString() bring ' to the str, which lead error in sqlite
    header_pair.push(name);
    header_pair.push(value);
    headers.push(header_pair);
  }});
  ...
  //------------------------------resp body file saver----------------------
  try{
    var contentType = httpChannel.getResponseHeader("Content-Type");
    if(httpChannel.responseStatus == "200" && contentType.indexOf("text/html") !== -1){
        var uri = httpChannel.URI.spec;
        getResponseBody(respEvent, uri);
    }
  }catch(e){
     //no content type filed. Do nothing.
  }
 
};

function getResponseHeaders(httpChannel){
  var headers = [];
  httpChannel.visitResponseHeaders({visitHeader: function(name, value) {
    var header_pair = [];
    header_pair.push(name);
  ...
  return removeQuoteMarkinStr(JSON.stringify(headers));
}

function getRequestHeaders(httpChannel){
  var headers = [];
  httpChannel.visitRequestHeaders({visitHeader: function(name, value) {
    var header_pair = [];
    header_pair.push(loggingDB.escapeString(name));
...
function getResponseBody(respEvent, uri) {
  var respBody;
  var newListener = new TracingListener();
  respEvent.subject.QueryInterface(Ci.nsITraceableChannel);
  newListener.originalListener = respEvent.subject.setNewListener(newListener);
  newListener.promiseDone.then(function() {
    respBody = newListener.responseBody; // get response body as a string
    var httpData = {};
    httpData["uri"] = uri;
    httpData["respBody"] = respBody;
    httpDataArr.push(httpData);
  }, function(aReason) {
    }).catch(function(aCatch) {
  });
}
...
exports.updateVisitAndCrawlID = function(_vid, _cid){
    visitID =_vid;
    crawlID = _cid;
}

exports.saveRespBodyToFile = function(finalUrl, respBodyPath){
  if(httpDataArr.length == 0)return;
  for(var i = 0; i < httpDataArr.length; i++){
    if(httpDataArr[i].uri == finalUrl){
      //replace / with * for valid filename
      var fileName = crawlID + "_" + visitID + "_" + finalUrl.replace(/\//g, '*');
      respBodySaver.saveToLocalFile(fileName, httpDataArr[i].respBody, respBodyPath);
      break; // only save the first match
    }
  }
  httpDataArr=[];
}
