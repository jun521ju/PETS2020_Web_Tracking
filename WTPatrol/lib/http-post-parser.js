...
var HttpPostParser = function(stream) {
  ...
  if (this.seekablestream instanceof components.interfaces.nsIMIMEInputStream) {
    ...
    this.hasheaders = true;
    this.body = -1;
  } else if (this.seekablestream instanceof components.interfaces.nsIInputStream) {
    // AJAX requests with "FormData" payload use nsIMultiplexInputStream
    this.seekablestream.QueryInterface(components.interfaces.nsIInputStream);
    this.hasheaders = true;
    this.body = -1;
  }
  else if (this.seekablestream instanceof components.interfaces.nsISeekableStream) {
    // AJAX requests with "FormData" payload use nsIMultiplexInputStream
    this.seekablestream.QueryInterface(components.interfaces.nsISeekableStream);
    this.hasheaders = true;
    this.body = -1;
  }else {
    // Let's keep an eye on unknown stream types, though we haven't seen any other stream types in our tests.
    console.log("seekablestream: " + this.seekablestream);
    loggingDB.logError("POST request parser: unknown stream type");
  }
}
...
HttpPostParser.prototype.parseEncodedFormData = function(formData, encodingType){
  ...
  try{
    ...
    loggingDB.logError("POST data is not parseable at Line 177 HttpPostParser.");
